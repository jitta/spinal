...
Spinal.prototype.provide = function(name, fn){
  if (this._methods[name])
    return this._methodsMiddleware[name].push(fn)
  if (this.initialized)
  ...
  this._methodsMiddleware[name] = []
  this._methodsMiddleware[name].push(fn)
  ...
  this._methods[name] = function(arg, callback){
    ...
    if (that._methodsMiddleware[name].length > 1) {
      var lastFn = that._methodsMiddleware[name].splice(-1, 1)[0]

      async.series(that._methodsMiddleware[name].map(function(_fn) {
        return function(cb) {
        ...
        lastFn(arg, reply, options)
      })
    }
    else {
      that._methodsMiddleware[name][0](arg, reply, options)
    }
  }
